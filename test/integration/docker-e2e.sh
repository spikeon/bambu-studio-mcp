...
# Default: single-plate calib project. flowrate-test-pass1.3mf is multi-plate and often crashes headless.
FIXTURE_PATH="${BAMBU_E2E_FIXTURE_PATH:-resources/calib/pressure_advance/auto_pa_line_single.3mf}"
...
curl -fsSL -o "${WORKDIR}/sample.3mf" "${BASE}/${FIXTURE_PATH}"
...
run_bs "model --info" --debug 2 --info /work/sample.3mf | tee "${WORKDIR}/out/info.log"
...
run_bs "slice + --arrange 1" \
  --debug 2 --arrange 1 --slice 0 --export-3mf /work/out/with_arrange.3mf /work/sample.3mf
require_min_bytes "${WORKDIR}/out/with_arrange.3mf"

run_bs "slice + --scale 0.92" \
  --debug 2 --scale 0.92 --slice 0 --export-3mf /work/out/with_scale.3mf /work/sample.3mf
